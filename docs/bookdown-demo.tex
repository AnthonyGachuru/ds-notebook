...
Another instantiation via Hadley Wickham is: import -\textgreater{} tidy -\textgreater{} understand {[} transform \textless{}-\textgreater{} visualize \textless{}-\textgreater{} model {]} -\textgreater{} communicate. As an abbreviation: ittvmcmd.

And the two combined: preparation -\textgreater{} import -\textgreater{} tidy -\textgreater{} understand {[} transform \textless{}-\textgreater{} visualize \textless{}-\textgreater{} model {]} -\textgreater{} {[}communicate + deploy + maintain{]}
...
\hypertarget{linear-regression-assumption-checks}{%
\section{Linear Regression Assumption Checks}\label{linear-regression-assumption-checks}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# general check}
\KeywordTok{plot}\NormalTok{(mdl)}

\CommentTok{# linearity check}
\KeywordTok{avPlots}\NormalTok{(mdl)}

\CommentTok{# Outlier Analysis}
\CommentTok{# influencePlot(mdl)}

\CommentTok{# multi-colinearity}
\KeywordTok{vif}\NormalTok{(mdl) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{data.frame}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{VIF =} \StringTok{`}\DataTypeTok{GVIF..1..2.Df..}\StringTok{`}\NormalTok{, }\DataTypeTok{VIF =}\NormalTok{ VIF}\OperatorTok{^}\DecValTok{2}\NormalTok{)}

\CommentTok{#  Autocorrelated Errors}
\KeywordTok{durbinWatsonTest}\NormalTok{(rgr_lr_st1)}

\CommentTok{# Normality}
\KeywordTok{shapiro.test}\NormalTok{(data)}
...
\CommentTok{# heteroskedasticity}
\CommentTok{# Breusch-Pagan (BP) test lmtest package}
\end{Highlighting}
\end{Shaded}
...
An attempt to apply the principles in \href{https://www.amazon.com/dp/B0030V0PEW/ref=dp-kindle-redirect?_encoding=UTF8\&btkr=1}{``The Checklist Manifesto''} to Data Science.
