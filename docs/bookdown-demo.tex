...
--Find tables with a given column name

SELECT TABLE\_SCHEMA, TABLE\_NAME --TABLE\_CATALOG, TABLE\_SCHEMA,
TABLE\_NAME, COLUMN\_NAME FROM INFORMATION\_SCHEMA.COLUMNS WHERE
COLUMN\_NAME LIKE `\%Test\%'

--get schema info SELECT table\_schema, table\_name, column\_name,
data\_type, character\_maximum\_length, is\_nullable, column\_default,
numeric\_precision, numeric\_scale FROM information\_schema.columns
WHERE table\_name = `Results' use this\_schema SELECT table\_schema,
table\_name, column\_name, data\_type, character\_maximum\_length,
is\_nullable, column\_default, numeric\_precision, numeric\_scale FROM
information\_schema.columns ORDER BY table\_schema, table\_name,
ordinal\_position
...
\item
  ...
  it. \#\#\# Outlier Detection
\end{itemize}

\url{https://www.rdocumentation.org/packages/DMwR/versions/0.4.1/topics/lofactor}
...
multiple stochastic regression imputation: mice
\protect\hyperlink{r}{R}, fancy impute
\protect\hyperlink{python}{Python}
...
\item
  swarmplot
\item
  \url{https://www.jumpingrivers.com/blog/styling-ggplot2-r-graphics/}
\item
...
\hypertarget{python}{\section{Python}\label{python}}
...
\hypertarget{r}{\section{R}\label{r}}
...
\item
  purrr::p/map\_dfr/c
\item
  date = date \%\textgreater{}\% as.POSIXct() \%\textgreater{}\%
  strptime(`\%Y-\%m-\%d') \%\textgreater{}\%
...
\subsection{Create List Column}\label{create-list-column}
...
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}
    \DataTypeTok{unq =} \KeywordTok{list}\NormalTok{(}\KeywordTok{unique}\NormalTok{(SId))}
\NormalTok{  )}

\NormalTok{eng_rivalries <-}\StringTok{ }\NormalTok{eng_rivalries }\OperatorTok{%>%}\StringTok{ }\KeywordTok{drop_na}\NormalTok{() }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{rivalry =} \KeywordTok{map2}\NormalTok{(team1_match, team2_match, }\ControlFlowTok{function}\NormalTok{(x, y) }\KeywordTok{c}\NormalTok{(x, y)))}
\end{Highlighting}
\end{Shaded}
...
\subsection{Tidy Evaluation Example}\label{tidy-evaluation-example}
...
\StringTok{    }\KeywordTok{filter}\NormalTok{(}
      \OperatorTok{!}\KeywordTok{is.na}\NormalTok{((}\OperatorTok{!!}\KeywordTok{as.symbol}\NormalTok{(}\StringTok{"col1"}\NormalTok{))),}
\NormalTok{      (}\OperatorTok{!!}\KeywordTok{as.symbol}\NormalTok{(}\StringTok{"col2"}\NormalTok{)) }\OperatorTok{==}\StringTok{ }\DecValTok{1}
\NormalTok{      ) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{select}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{4}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\subsection{DT Options}\label{dt-options}
