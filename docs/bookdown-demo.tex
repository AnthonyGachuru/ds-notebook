...
Linear: When you're predicting a continuous value.
...
Breusch-Pagan (BP) test lmtest package: is used to test for
...
If H\_0 is true then the population mean of Y would be ? 0 no matter
what the value of X. (X has no effect on Y.)

F-Test: H\_0 says that all/some coefficients are zero versus H\_a which
says that there is at least one non-zero coefficient. If h0 fail to
reject than F value close to 1, else F value greater than 1.
...
model complexity into consideration which makes it prone to overfitting.
Use adjusted R-squared instead. Additional predictors only make this
increase is they are statistically significant. It's easy to show that
given no other data for a set of numbers, the predictor that minimises
the MAE is the median, while the predictor that minimises the MSE is the
mean.

\begin{itemize}
\item
  Compare rmse to sd of data for sense of how good it is. (If rsme
  \textless{} sd then good.)
\item
  Shapiro-Wilk Test for Normality: H\_0 says the data is normally
  distributed while H\_a says it is not. To run this test in R, the
...
\addcontentsline{toc}{chapter}{Frameworks}
...
\section{Train-Test Split, Folds, \&
...
\addcontentsline{toc}{chapter}{Code Snippets}
...
\section{Shiny Reactive}\label{shiny-reactive}
...
\section{Custom Theme}\label{custom-theme}
...
\section{Create Factor}\label{create-factor}
...
\chapter*{General}\label{general-3}
...
\section{Other}\label{other-4}

\href{https://cran.r-project.org/web/packages/infer/infer.pdf}{Inference
package}
