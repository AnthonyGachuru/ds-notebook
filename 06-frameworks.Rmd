...
keras: declare model | compile with loss, optimizer, and scoring metric | fit
...
Run a glmer with poisson family error term, predicting count as a function of fixed effects age and year, and include year as a random effect of county as well.
...
ageMotherModelRandomCorrelated <- lmer( BirthRate ~ AverageAgeofMother + (AverageAgeofMother | State),
 ...
 assumed that observations within each group were correlated.
...
The [infer](https://cran.r-project.org/web/packages/infer/infer.pdf) library has  the main verbs: specify() variables, hypothesize(), generate() data, calculate() metrics, visualize() results
...
‘Classical SNA’ is mainly about descriptive network statistics: proximity, similarity, centrality, brokerage, positional measures, equivalence, etc.
